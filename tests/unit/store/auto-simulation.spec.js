...
describe('Auto-Simulation', () => {
	...
	describe('Debounced Simulation Trigger', () => {
		beforeEach(() => {
			jest.useFakeTimers();
		});
		...
		test('should debounce multiple rapid parameter changes', async () => {
			// Initialize circuit with a component
			await store.dispatch('circuit/newFile');
			const resistor = new Resistor(10, 10);
			await store.dispatch('circuit/addComponent', resistor);
			...
			await store.dispatch('circuit/updateComponent', {
				componentId: resistor.id,
				updates: { parameters: { resistance: 1000 } },
			});

			await store.dispatch('circuit/updateComponent', {
				componentId: resistor.id,
				updates: { parameters: { resistance: 2000 } },
			});

			await store.dispatch('circuit/updateComponent', {
				componentId: resistor.id,
				updates: { parameters: { resistance: 3000 } },
			});

			// Advance time by less than debounce delay
			jest.advanceTimersByTime(200);

			// Simulation should not have been triggered yet
			expect(mockRunSimulation).not.toHaveBeenCalled();

			// Advance time to complete debounce delay
			jest.advanceTimersByTime(100);

			// Simulation should have been triggered only once
			expect(mockRunSimulation).toHaveBeenCalledTimes(1);
		});

		test('should use 300ms debounce delay', async () => {
			// Initialize circuit with a component
			await store.dispatch('circuit/newFile');
			const resistor = new Resistor(10, 10);
			await store.dispatch('circuit/addComponent', resistor);
			...
			await store.dispatch('circuit/updateComponent', {
				componentId: resistor.id,
				updates: { parameters: { resistance: 5000 } },
			});

			// Advance time by 299ms (just before debounce completes)
			jest.advanceTimersByTime(299);

			// Simulation should not have been triggered yet
			expect(mockRunSimulation).not.toHaveBeenCalled();

			// Advance time by 1ms to complete debounce delay
		...
		test('should reset debounce timer on each parameter change', async () => {
			// Initialize circuit with a component
			await store.dispatch('circuit/newFile');
			const resistor = new Resistor(10, 10);
			await store.dispatch('circuit/addComponent', resistor);
			...
			// Make second parameter change (should reset timer)
			await store.dispatch('circuit/updateComponent', {
				componentId: resistor.id,
				updates: { parameters: { resistance: 2000 } },
			});

			// Advance time by 200ms (total 400ms from first change, but only 200ms from second)
			jest.advanceTimersByTime(200);

			// Simulation should not have been triggered yet
			expect(mockRunSimulation).not.toHaveBeenCalled();

			// Advance time by 100ms to complete debounce from second change
			jest.advanceTimersByTime(100);

			// Simulation should have been triggered once
			expect(mockRunSimulation).toHaveBeenCalledTimes(1);
		});
	});
